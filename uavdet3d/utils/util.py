...
def draw_box9d_on_image(boxes9d, image, img_width=1920., img_height=1080., color=(255, 0, 0), intrinsic_mat=None, extrinsic_mat=None, distortion_matrix=None, offset=np.array([0,0,0])):
    ...
    for box in boxes9d:
        ...
        for edge in edges:
            start, end = edge
            cv2.line(image, tuple(corners_2d[start]), tuple(corners_2d[end]), color, 10)
...
def vi():
    ...
    im_path_name = 'JPEGImages'
    label_path_name = 'labels'
    label6d_name = 'labels_yolo6d'

    root_path = r'D:\data\MAV6D\phantom4'

    all_scene = os.listdir(os.path.join(root_path, im_path_name))

    intrinsic = np.array([[1979.4, 0.3984, 976.8189, ],  #
             [0.0, 1979.1, 533.9717, ],#
             [0.0, 0.0, 1.0, ],])
    distortion_matrix = np.array([-0.2306, 0.1497, -0.00089582, -0.00086321, -0.0522086113480487])
    extrinsic = np.eye(4)

    img_width = 1920.
    img_hight = 1080.

    for i,scene_name in enumerate(all_scene) :

        all_seq = os.listdir(os.path.join(root_path,im_path_name,scene_name))


        for seq_name in all_seq:

            all_frame = os.listdir(os.path.join(root_path,im_path_name,scene_name, seq_name))

            num = np.array(list(map(lambda x:float(x.split('.')[0]), all_frame)))

            sor = np.argsort(num)

            for k_i in range(len(all_frame) ) :
                frame_name = all_frame[sor[k_i]]

                each_im_path = os.path.join(root_path,im_path_name,scene_name, seq_name, frame_name)
                each_label_path = os.path.join(root_path,label_path_name,scene_name, seq_name, frame_name.replace('jpg', 'txt'))
                each_label6d_path = os.path.join(root_path,label6d_name,scene_name, seq_name, frame_name.replace('jpg', 'txt'))

                rotation_matrix, translation_matrix = read_truth_Rt(each_label_path)
                ...
                box9d = np.concatenate([translation_matrix,np.array([.240, .240, .230]),euler_angles],0)

                boxes9d = [box9d]

                image = cv2.imread(each_im_path)
                ...
                pts = box9d_to_key_points(boxes9d,offset=np.array([0.2,0.2,0.]))

                image = draw_points_on_image(pts, image,
                                            img_width=img_width,
                                            img_height=img_hight,
                                            color=(0,0,255),
                                            intrinsic_mat = intrinsic,
                ...
                # im_pts = pts3d_to_im_pts(pts=pts,
                #                          intrinsic_mat = intrinsic,
                #                          extrinsic_mat = extrinsic,
                #                          distortion_matrix=distortion_matrix)
                #
                # boxes9d = PnP_im_points_to_boxes9d(im_points=im_pts, size = np.array([.240, .240, .230]),intrinsic_mat=intrinsic,distortion_matrix=distortion_matrix)
                #
                # image = draw_box9d_on_image(boxes9d, image,
                #                             img_width=img_width,
                #                             img_height=img_hight,
    ...
    for i,scene_name in enumerate(all_scene) :
        ...
        for seq_name in all_seq:
            ...
            for k_i in range(len(all_frame) ) :
                ...
                # cv2.waitKey(0)
                #
    all_dis = np.concatenate(all_dis)
    print(all_dis)
    dis = np.linalg.norm(all_dis,axis=-1)
    print(dis.max())
